...
参考答案是从生成的ucroe.img反推需要的文件，逐层向上推理。这对于对代码逻辑比较不熟悉的我不是很友好，因此我采用的是从上而下看输出指令的方式确定makefile的逻辑，再整理清楚逻辑后对makefile文件进行分析，对我自己而言更容易理解。

### 练习2

第一问：相比于参考答案，我删除了continue语句，如果不删除continue语句，无法正确的跳到第一条ljmp指令

第四问：增加了测试点，参考答案没有实现第四问

### 练习3

相比参考答案，我将代码分成了4部分，正好对应了bootloader 进入保护模式的四个步骤。

同时对四个问题进行了回答，参考答案中没有列出

### 练习4

相比参考答案多了运行调试过程，观察了变量的值，更好的分析了bootloader加载ELF格式文件的过程

### 练习5

我对最后一行的参数含义进行了更加细致的解释
...
- 切换到保护模式，启用分段机制
- 读磁盘中ELF执行文件格式的`ucore`操作系统到内存
- 显示字符串信息
- 将控制权移交给`ucore`系统

在lab1中，实现文件在`boot\asm.h`,`boot\bootasm.S`,`boot\bootmain.c`中

### 地址空间

分段机制涉及4个关键内容：逻辑地址、物理地址、段描述符表、段描述符、段选择子

**逻辑地址空间** ：应用程序角度，逻辑地址空间就是应用程序员编程所用到的地址空间

**物理地址空间**：操作系统角度，CPU、内存硬件和各种外设是它主要管理的硬件资源而内存硬件和外设分布在物理地址空间中

在上周和上上周的课程中我们学了相关知识，并且完成了相应的课后练习：如何进行虚拟地址和物理地址的转换